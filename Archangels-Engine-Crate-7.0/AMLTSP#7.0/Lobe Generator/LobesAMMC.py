...
def create_profile(duration, lift, trim_factor, sigma, vol, res, at_lift, ramp_steepness, ramp_pos, lift_significant_fraction, cos):
    ...
    if at_lift != 0:
        ...
        x = mult(x, np.min(x) / interpolated_x) # scale x according to @ lift value
        if np.min(x) < -180:
...
def create_valve_path_flat_tappet(xs: np.array, ys: np.array, base: float) -> np.array:
    ...
    for i, x in enumerate(radians_xs):
        current_cosines = np.cos(radians_xs - x)
        ry[i] = np.max(current_cosines * lifts_with_base) - base
    return ry


def create_valve_path_roller_tappet(xs: np.array, ys: np.array, base: float, roller_base: float) -> np.array:
    radians_xs = np.radians(xs)
    base_plus_ys = base + np.array(ys)
    ry = np.zeros_like(xs)
    for i, x in enumerate(radians_xs):
        current_angles = radians_xs - x
        curr_sin = np.sin(current_angles) * base_plus_ys
        curr_cos = np.cos(current_angles) * base_plus_ys
...
def plot(ax, x, y, vly, angle, base, dur_at_lift, rtr, name, at_lift, color):
    ...
    if at_lift != 0:
        ax.plot([0, np.deg2rad(dur_at_lift / 4 + angle)], [0, base + at_lift], linestyle='dashdot', color=color, linewidth=0.5, label=f'{name} duration @ {at_lift:.3f} mm') # lobe @ lift start
        ax.plot([0, np.deg2rad(-dur_at_lift / 4 + angle)], [0, base + at_lift], linestyle='dashdot', color=color, linewidth=0.5) # lobe @ lift end
    legend_angle = -45
...
def read_basic_params(fpath):
    ...
    with open(fpath, 'r', encoding='utf-8') as f:
        mr_content = f.read()
    p_ivl = parse.search("\nlabel IV_L({:f})", mr_content)
    p_evl = parse.search("\nlabel EV_L({:f})", mr_content)
    p_ivo = parse.search("\nlabel IV_O({:f} * units.deg)", mr_content)
    p_ivc = parse.search("\nlabel IV_C({:f} * units.deg)", mr_content)
    p_evo = parse.search("\nlabel EV_O({:f} * units.deg)", mr_content)
    p_evc = parse.search("\nlabel EV_C({:f} * units.deg)", mr_content)
...
def lobes(fpath, arch, volume_limit, at_lift_limit):
    ...
    allowed_vol = np.linspace(0.01, 1.0, 100)
